...
router.route("/signup").post(async (req, res) => {
  ...
  const link = `https://money-manager7.herokuapp.com/users/twostepverification/${token}`;
  const message = `<h3>Greetings ${fname} !!!</h3>
<p>Welcome to the world of Money Manager</p>
<p>Using our services you can Track Your Savings and Expense</p>
<a href=${link}>Click the link to complete two step verification</a>
<p>Two step verification is mandatory to Signin</p>
<p>Regards,</p>
<p>Money Manager Team</p>`;
...
router.route("/twostepverification/:id").get(async (req, res) => {
  ...
  try {
    ...
    res.redirect(`https://nifty-heyrovsky-ac9f1f.netlify.app/activationmessage`);
  } catch (err) {
...
router.route("/forgotpassword").post(async (req, res) => {
  ...
  const link = `https://money-manager7.herokuapp.com/users/forgotpassword/verify/${token}`;
...
router.route("/forgotpassword/verify/:id").get(async (req, res) => {
  ...
  return res.redirect(`https://nifty-heyrovsky-ac9f1f.netlify.app/resetpassword/${id}`);
});
