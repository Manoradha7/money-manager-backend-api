...
async function createUser(
  ...
  return await client.db("money").collection("users").insertOne({
    fname,
    lname,
    username,
    email,
...
async function getUser(userData) {
  return await client.db("money").collection("users").findOne(userData);
}

async function getUserByEmail(email) {
  return await client.db("money").collection("users").findOne({ email });
}
...
async function UpdateUserById(id, data) {
  return await client.db("money").collection("users").updateOne({ _id: ObjectId(id) }, { $set: data });
}
